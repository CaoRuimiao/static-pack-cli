        ...
        childProcess.spawnSync(
            `npx webpack-dev-server ` +
                `${Object.entries(envConfig)
                    ...
                    .join(' ')} ` +
                `--config ./node_modules/static-pack-cli/webpack.config.js`,
...
function runProduction(envConfig) {
    childProcess.spawnSync(
        `npx webpack --config ./node_modules/static-pack-cli/webpack.config.js ` +
            `${Object.entries(envConfig)
                .map(([key, value]) => `--env.${key}="${value}"`)
                .join(' ')} ` +
            ` && npx gulp --gulpfile ./node_modules/static-pack-cli/gulpfile.js`,
        { stdio: 'inherit', shell: true }
    );
}
