...
const configPath = path.resolve('./static-pack-cli.config.js');
...
if (!fs.existsSync(configPath)) {
    throw Error(
        '没有找到static-pack-cli的配置文件，请在项目目录下创建static-pack-cli.config.js文件，导出一个json对象'
    );
}
const config = require('../lib/config');
if (!config.distDir) {
    console.warn(
        '没有设置static-pack-cli.config.js中的 distDir 属性，默认使用`.dist/`作为输出目录。'
    );
}
